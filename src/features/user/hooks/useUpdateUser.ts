...
export function useUpdateUser(imagesUrl: string[], user: any, ){
    ...
    const handleUpdateUser = async (data?: EditHeadingForm | EditAboutForm) => {
        try {
          ...
          if(data){
            if ('username' in data && (data.username === user?.username)) {
                console.log('No username were changed.');
                delete data.username;
            }
    
            if('title' in data && (data.title === user?.title)){
                console.log('No title were changed');
                delete data.title;
            }
        
            if('bio' in data && (data.bio === user?.bio)){
