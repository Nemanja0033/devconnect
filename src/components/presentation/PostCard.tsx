...
const PostCard = ({image, title, id, comments, likes}: PostType ) => {
  return (
    <div className="lg:w-[70%] w-full h-[450px] rounded-md border border-gray-100 shadow-md p-3 flex-row">
            ...
            <div className="flex justify-center m-3">
              <Link className="bg-purple-900 rounded-full p-3 flex items-center gap-1 hover:scale-105 transition-all text-white shadow-lg" href={`/posts/${id}`}>Read Article <ArrowUpRightFromSquareIcon /></Link>
            </div>
    </div>
  )
}
